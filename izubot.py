...
TOKEN = "8538560833:AAEAxF39-5HEfhvkGzD-fCVumALG5NvnHzA"
ADMIN_ID = 8755093471
KEY_PREFIX = "!"
...
BOT_DISPLAY_NAME = " ᴘʀᴇᴍɪᴜᴍ sᴇᴀʀᴄʜᴇʀ"
...
class DataDomeGenerator:
    ...
    def get_new_datadome(self):
        ...
        try:
            ...
            if response_json.get('status') == 1000 and 'cookie' in response_json:
    ...
    if is_at_least_role(user_id, "owner"):
        ...
        welcome_msg = (
         ...
         welcome_msg = (
            ...
            "For any assistance, please contact: @meruemizu"
        )
        ...
        welcome_msg = (
            ...
            "Need premium access? Contact: @meruemizu"
        )
...
async def handle_enter_key(update: Update, context: CallbackContext):
    ...
    if key in ACCESS_KEYS:
        ...
        await update.message.reply_text(
            ...
            "🆕 **ɢᴇᴛ ɴᴇᴡ ᴀᴄᴄᴇss**\n"
            "ᴄᴏɴᴛᴀᴄᴛ: @meruemizu ғᴏʀ ᴀ ɴᴇᴡ ᴘʀᴇᴍɪᴜᴍ ᴋᴇʏ.",
            parse_mode="Markdown"
        )
        AWAITING_KEY_INPUT.discard(user_id)
    else:
        await update.message.reply_text(
            ...
            "👨💻 **sᴜᴘᴘᴏʀᴛ**\n"
            "ᴄᴏɴᴛᴀᴄᴛ: @meruemizu ғᴏʀ ᴀssɪsᴛᴀɴᴄᴇ.",
            parse_mode="Markdown"
        )
...
async def prompt_for_key(update: Update, context: CallbackContext):
    ...
    message_text = (
        ...
        "`zeropremium-abc789`\n\n"
        "🔓 **ɴᴇᴇᴅ ᴀᴄᴄᴇss?**\n"
        "ᴄᴏɴᴛᴀᴄᴛ @meruemizu ᴛᴏ ᴀᴄǫᴜɪʀᴇ ʏᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴋᴇʏ."
    )
...
async def generate_file(update: Update, context: CallbackContext):
    ...
    try:
        ...
        result_filename = f"♨️zero_ᴘʀᴇᴍɪᴜᴍ_{clean_game_name}_{datetime.datetime.now().strftime('%m%d_%H%M')}.txt"
...
async def database_status(update: Update, context: CallbackContext):
    ...
    status_text += f"• Estimated Total Generations Possible (at 1000 lines/gen): **{total_lines // 1000:,}**\n"
...
async def show_help(update: Update, context: CallbackContext):
    ...
    help_text = (
        ...
        "➢ ᴏʙᴛᴀɪɴ ʏᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss ᴋᴇʏ ғʀᴏᴍ **@meruemizu**.\n"
        ...
        "➣ ɢᴇɴᴇʀᴀᴛᴇs **1000 ʟɪɴᴇs** ᴘᴇʀ ʀᴇǫᴜᴇsᴛs.\n"
        ...
        "⌫ ғᴏʀ ᴀɴʏ ғᴜʀᴛʜᴇʀ sᴜᴘᴘᴏʀᴛ ᴏʀ ɪɴǫᴜɪʀɪᴇs, ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴛʜᴇ ᴀᴅᴍɪɴɪsᴛʀᴀᴛᴏʀ **@meruemizu**."
    )
    keyboard = [[InlineKeyboardButton("⬅️ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴍᴇɴᴜ", callback_data="back_to_main_menu")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    if not has_access(user_id):
        if update.callback_query:
            ...
            await current_message.edit_text(
                ...
                "You need active premium access to use the Python Encryptor. Please use an access key or contact @meruemizu to purchase access.",
                parse_mode="Markdown"
            )
        else:
            await current_message.reply_text(
...
async def datadome_info(update: Update, context: CallbackContext):
    ...
    await current_message.edit_text(
        ...
        "For technical support: @meruemizu",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
